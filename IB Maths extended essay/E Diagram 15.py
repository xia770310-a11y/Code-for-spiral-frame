...
# Define the time range and create time array
t_start, t_end = 0, 0.005
...
# Function 1: Using Jacobi elliptic cosine (cn)
def function1(x, t):
    # For scipy, ellipj(u, m) where m = k^2
    # Here k = 0.701, so m = 0.701^2
    m = 0.701**2
    u = 62200 * t
...
# Function 2: Absolute value of cosine times sine
def function2(x, t):
...
def update(frame):
    ...
    # Update timer box with current time
    timer_box.set_text(f'Time: {t:.5f} s')
    
    return line1, line2, timer_box

animation = FuncAnimation(fig, update, frames=len(t_values), 
